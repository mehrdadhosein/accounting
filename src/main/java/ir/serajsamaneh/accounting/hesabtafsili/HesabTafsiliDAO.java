...
public class HesabTafsiliDAO extends BaseHibernateDAO<HesabTafsiliEntity, Long> {
	...
	/*
	 * public Long getMaxHesabTafsiliCode(OrganEntity currentOrgan, SaalMaaliEntity
	 * activeSaalMaaliEntity) { Map<String, Object> filter = new HashMap<String,
	 * Object>(); // filter.put("saalMaali.id@eq",
	 * currentUserSaalMaaliEntity.getId()); filter.put("organId@eq",
	 * currentOrgan.getId()); filter.put("saalMaali.id@eq",
	 * activeSaalMaaliEntity.getId()); addLocationFilter(filter);
	 * 
	 * Criteria criteria = getEmptyCriteria(); addFilterCriteria(filter, criteria);
	 * 
	 * // criteria.setProjection(Projections.projectionList().add( //
	 * Projections.max(KalaEntity.PROP_CODE)));
	 * 
	 * List<HesabTafsiliEntity> list =
	 * criteria.setFlushMode(FlushMode.MANUAL).list(); // .uniqueResult();
	 * 
	 * Long maxCode = 0l; for (HesabTafsiliEntity kalaEntity : list) { Long codeLong
	 * = kalaEntity.getCode(); if(codeLong!=null){ // Long codeLong =
	 * Long.valueOf(codeStr); maxCode = Math.max(maxCode.longValue(),
	 * codeLong.longValue()); }
	 * 
	 * } // for (HesabTafsiliEntity kalaEntity : list) { // String codeStr =
	 * kalaEntity.getCode(); // if(StringUtils.hasText(codeStr)){ // Long codeLong =
	 * Long.valueOf(codeStr); // maxCode = Math.max(maxCode.longValue(),
	 * codeLong.longValue()); // } // // }
	 * 
	 * return maxCode;
	...
	public Long getMaxHesabTafsiliCode(HesabClassificationEntity hesabClassification, Integer kalaCodeCharactersNumber,
			Long organId, SaalMaaliEntity saalMaaliEntity) {
...
//			}
//			
			String classificationCode = (hesabClassification != null && hesabClassification.getId() != null)
					? hesabClassification.getCode().toString()
					: "";
...
//			String codeNumberOnly = codeStr.replaceAll("-", "");
		...
		return Long.valueOf(maxCodeStr);
	}

	private String createCode(HesabClassificationEntity hesabClassification, Long maxCode,
			Integer kalaCodeCharactersNumber) {
		if (hesabClassification != null && hesabClassification.getId() != null) {
			String maxCodeStr = "";
			...
			if (kalaCodeCharactersNumber > 0) {
				...
				if (zeroDigitCount < 0)
					throw new FatalException(
							SerajMessageUtil.getMessage("HesabTafsili_hesabTafsiliCodeCharactersNumberIsShort"));
