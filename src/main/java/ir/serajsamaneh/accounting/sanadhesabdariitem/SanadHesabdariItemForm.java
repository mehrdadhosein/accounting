	...
	public SanadHesabdariItemEntity getEntity() {
		if (entity == null) {
			...
			if (StringUtil.hasText(selectedOrganIdStr))
				setSelectedOrganId(Long.valueOf(selectedOrganIdStr));
...
//	}
	...
	public void setSanadhesabdariItemFilter(Map<String, Object> sanadhesabdariItemFilter) {
...
//		OrganEntity selectedOrgan = getSelectedOrganId()!=null ? getOrganService().load(getSelectedOrganId()) : null;
	...
	private void createDaftarSummaryDaftarLocalFilter(Long organId) {
	...
	private void createDaftarLocalFilter(Long organId) {
		populateFilterFromRequest();
	...
	private void createDaftarHierarchicalFilter() {
		...
		getFilter().put("sanadHesabdari.organId@in", topOrganList);
		// getFilter().put("sanadHesabdari.organ.code@startlk",
...
//		setRowsPerPage(1000);
		createDaftarSummaryDaftarLocalFilter(getCurrentOrganVO().getId());
		return getDataModel();
	}
...
//		OrganEntity selectedOrgan = getSelectedOrganId()!=null ? getOrganService().load(getSelectedOrganId()) : null;
	...
	public SaalMaaliEntity getSelectedSaalMaali() {
		if (getSearchBySaalMaali())
			if (getSaalMaaliId() != null)
				return saalMaaliService.load(getSaalMaaliId());
			else
				return null;
	...
	public List<SanadHesabdariItemVO> getTarazKolAzmayeshi() {
		...
		setSearchAction(true);
		List<SanadHesabdariItemVO> tarazKolAzmayeshi = extractTarazKolAzmayeshi(getCurrentOrganVO().getId());
	...
	public List<SanadHesabdariItemVO> getHierarchicalTarazKolAzmayeshi() {
		...
		setSearchAction(true);
//		OrganEntity selectedOrgan = getSelectedOrganId()!=null ? getOrganService().load(getSelectedOrganId()) : null;
		List<SanadHesabdariItemVO> tarazKolAzmayeshi = extractTarazKolAzmayeshi(getSelectedOrganId());
		return tarazKolAzmayeshi;
	}

	private List<SanadHesabdariItemVO> extractTarazKolAzmayeshi(Long organId) {
		populateFilterFromRequest();
		List<SanadHesabdariItemVO> tarazKolAzmayeshi = getMyService().getTarazKolAzmayeshi(getSelectedSaalMaali(),
				getFromDate(), getToDate(), getHesabKolIds(), getMoeenIds(), getTafsiliIds(), getTafsiliTwoIds(),
				getAccountingMarkazIds(), null, organId, getFromSerial(), getToSerial(), getSanadhesabdariItemFilter(),
				getDisplayZeroMandehInTaraz(), getBeforeClosingAccounts(), getNumberOfDecimals());
	...
	public List<SanadHesabdariItemVO> getTarazMoeenAzmayeshi() {
		if (!FacesContext.getCurrentInstance().getRenderResponse())
			return new ArrayList<SanadHesabdariItemVO>();
		setSearchAction(true);
		List<SanadHesabdariItemVO> tarazMoeenAzmayeshi = extractTarazMoeenAzmayeshi(getCurrentOrganVO().getId());
	...
	public List<SanadHesabdariItemVO> getHierarchicalTarazMoeenAzmayeshi() {
		...
		setSearchAction(true);
//		OrganEntity selectedOrgan = getSelectedOrganId()!=null ? getOrganService().load(getSelectedOrganId()) : null;
		List<SanadHesabdariItemVO> tarazMoeenAzmayeshi = extractTarazMoeenAzmayeshi(getSelectedOrganId());
		return tarazMoeenAzmayeshi;
	}

	private List<SanadHesabdariItemVO> extractTarazMoeenAzmayeshi(Long organId) {
		populateFilterFromRequest();
		List<SanadHesabdariItemVO> tarazMoeenAzmayeshi = getMyService().getTarazMoeenAzmayeshi(getSelectedSaalMaali(),
				getFromDate(), getToDate(), getHesabKolIds(), getMoeenIds(), getTafsiliIds(), getTafsiliTwoIds(),
				getAccountingMarkazIds(), null, organId, getFromSerial(), getToSerial(), getSanadhesabdariItemFilter(),
				getDisplayZeroMandehInTaraz(), getNumberOfDecimals());
	...
	public List<SanadHesabdariItemVO> getTarazTafsiliAzmayeshi() {
		...
		if (!FacesContext.getCurrentInstance().getRenderResponse())
			return new ArrayList<SanadHesabdariItemVO>();
		setSearchAction(true);
		List<SanadHesabdariItemVO> tarazTafsiliAzmayeshi = extractTarazTafsiliAzmayeshi(getCurrentOrganVO().getId());
	...
	public List<SanadHesabdariItemVO> getTarazTafsiliAzmayeshiTwo() {
		...
		if (!FacesContext.getCurrentInstance().getRenderResponse())
			return new ArrayList<SanadHesabdariItemVO>();
		setSearchAction(true);
		List<SanadHesabdariItemVO> tarazTafsiliAzmayeshiTwo = extractTarazTafsiliAzmayeshiTwo(
	...
	public List<SanadHesabdariItemVO> getHierarchicalTarazTafsilAzmayeshi() {
		...
		setSearchAction(true);
//		OrganEntity selectedOrgan = getSelectedOrganId()!=null ? getOrganService().load(getSelectedOrganId()) : null;
		List<SanadHesabdariItemVO> tarazTafsiliAzmayeshi = extractTarazTafsiliAzmayeshi(getSelectedOrganId());
		return tarazTafsiliAzmayeshi;
	}
...
//		System.out.println(FacesContext.getCurrentInstance().getRenderResponse());
		if (!FacesContext.getCurrentInstance().getRenderResponse())
			return new ArrayList<SanadHesabdariItemVO>();
		setSearchAction(true);
		List<SanadHesabdariItemVO> tarazAccountingMarkazAzmayeshi = extractTarazAccountingMarkazAzmayeshi(
				getCurrentOrganVO().getId());
...
//		System.out.println(FacesContext.getCurrentInstance().getRenderResponse());
		...
		setSearchAction(true);
		List<SanadHesabdariItemEntity> tarazAccountingMarkazAzmayeshi = extractTarazAccountingMarkazShenavarAzmayeshi(
				getCurrentOrganVO().getId());
		return tarazAccountingMarkazAzmayeshi;
	}
...
//		OrganEntity selectedOrgan = getSelectedOrganId()!=null ? getOrganService().load(getSelectedOrganId()) : null;
	...
	private List<SanadHesabdariItemVO> extractTarazTafsiliAzmayeshi(Long organId) {
		populateFilterFromRequest();
		List<SanadHesabdariItemVO> tarazTafsiliAzmayeshi = getMyService().getTarazTafsiliAzmayeshi(
				getSelectedSaalMaali(), getFromDate(), getToDate(), getHesabKolIds(), getMoeenIds(), getTafsiliIds(),
				getTafsiliTwoIds(), getAccountingMarkazIds(), null, organId, getFromSerial(), getToSerial(),
				getSanadhesabdariItemFilter(), getDisplayZeroMandehInTaraz(), getNumberOfDecimals());
	...
	private List<SanadHesabdariItemVO> extractTarazTafsiliAzmayeshiTwo(Long organId) {
		populateFilterFromRequest();
		List<SanadHesabdariItemVO> tarazTafsiliAzmayeshiTwo = getMyService().getTarazTafsiliAzmayeshiTwo(
				getSelectedSaalMaali(), getFromDate(), getToDate(), getHesabKolIds(), getMoeenIds(), getTafsiliIds(),
				getTafsiliTwoIds(), getAccountingMarkazIds(), null, organId, getFromSerial(), getToSerial(),
				getSanadhesabdariItemFilter(), getDisplayZeroMandehInTaraz(), getNumberOfDecimals());
...
//	}
	...
	private List<SanadHesabdariItemVO> extractTarazAccountingMarkazAzmayeshi(Long organId) {
		populateFilterFromRequest();
		List<SanadHesabdariItemVO> tarazAccountingMarkazAzmayeshi = getMyService().getTarazAccountingMarkazAzmayeshi(
				getSelectedSaalMaali(), getFromDate(), getToDate(), getHesabKolIds(), getMoeenIds(), getTafsiliIds(),
				getTafsiliTwoIds(), getAccountingMarkazIds(), null, organId, getFromSerial(), getToSerial(),
				getSanadhesabdariItemFilter(), getDisplayZeroMandehInTaraz(), getBeforeClosingAccounts(),
				getNumberOfDecimals());
	...
	private List<SanadHesabdariItemEntity> extractTarazAccountingMarkazShenavarAzmayeshi(Long organId) {
		populateFilterFromRequest();
		List<SanadHesabdariItemEntity> tarazAccountingMarkazAzmayeshi = getMyService()
				.getTarazAccountingMarkazShenavarAzmayeshi(getSelectedSaalMaali(), getFromDate(), getToDate(),
						getHesabKolIds(), getMoeenIds(), getTafsiliIds(), getTafsiliTwoIds(), getAccountingMarkazIds(),
						null, organId, getFromSerial(), getToSerial(), getSanadhesabdariItemFilter(),
						getDisplayZeroMandehInTaraz(), getBeforeClosingAccounts(), getNumberOfDecimals());
	...
	public List<SanadHesabdariItemVO> getTarazKolDarayi() {
		return getMyService().getTarazKolAzmayeshi(getSelectedSaalMaali(), getFromDate(), getToDate(), getHesabKolIds(),
				getMoeenIds(), getTafsiliIds(), getTafsiliTwoIds(), getAccountingMarkazIds(), HesabTypeEnum.ASSET,
				getCurrentOrganVO().getId(), getFromSerial(), getToSerial(), getSanadhesabdariItemFilter(),
				getNumberOfDecimals());
	}

	public List<SanadHesabdariItemVO> getTarazKolBedehi() {
		return getMyService().getTarazKolAzmayeshi(getSelectedSaalMaali(), getFromDate(), getToDate(), getHesabKolIds(),
				getMoeenIds(), getTafsiliIds(), getTafsiliTwoIds(), getAccountingMarkazIds(), HesabTypeEnum.LIABILITY,
				getCurrentOrganVO().getId(), getFromSerial(), getToSerial(), getSanadhesabdariItemFilter(),
				getNumberOfDecimals());
	}
	...
	public List<SanadHesabdariItemVO> getTarazKolSarmaye() {
		return getMyService().getTarazKolAzmayeshi(getSelectedSaalMaali(), getFromDate(), getToDate(), getHesabKolIds(),
				getMoeenIds(), getTafsiliIds(), getTafsiliTwoIds(), getAccountingMarkazIds(), HesabTypeEnum.EQUITY,
				getCurrentOrganVO().getId(), getFromSerial(), getToSerial(), getSanadhesabdariItemFilter(),
				getNumberOfDecimals());
	}

	public List<SanadHesabdariItemVO> getTarazMoeenDarayi() {
		return getMyService().getTarazMoeenAzmayeshi(getSelectedSaalMaali(), getFromDate(), getToDate(),
				getHesabKolIds(), getMoeenIds(), getTafsiliIds(), getTafsiliTwoIds(), getAccountingMarkazIds(),
				HesabTypeEnum.ASSET, getCurrentOrganVO().getId(), getFromSerial(), getToSerial(),
				getSanadhesabdariItemFilter(), getBeforeClosingAccounts(), getNumberOfDecimals());
	}

	public List<SanadHesabdariItemVO> getTarazMoeenBedehi() {
		return getMyService().getTarazMoeenAzmayeshi(getSelectedSaalMaali(), getFromDate(), getToDate(),
				getHesabKolIds(), getMoeenIds(), getTafsiliIds(), getTafsiliTwoIds(), getAccountingMarkazIds(),
				HesabTypeEnum.LIABILITY, getCurrentOrganVO().getId(), getFromSerial(), getToSerial(),
				getSanadhesabdariItemFilter(), getBeforeClosingAccounts(), getNumberOfDecimals());
	}
	...
	public List<SanadHesabdariItemVO> getTarazMoeenSarmaye() {
		return getMyService().getTarazMoeenAzmayeshi(getSelectedSaalMaali(), getFromDate(), getToDate(),
				getHesabKolIds(), getMoeenIds(), getTafsiliIds(), getTafsiliTwoIds(), getAccountingMarkazIds(),
				HesabTypeEnum.EQUITY, getCurrentOrganVO().getId(), getFromSerial(), getToSerial(),
				getSanadhesabdariItemFilter(), getBeforeClosingAccounts(), getNumberOfDecimals());
	}

	public List<SanadHesabdariItemVO> getTarazTafsiliDarayi() {
		return getMyService().getTarazTafsiliAzmayeshi(getSelectedSaalMaali(), getFromDate(), getToDate(),
				getHesabKolIds(), getMoeenIds(), getTafsiliIds(), getTafsiliTwoIds(), getAccountingMarkazIds(),
				HesabTypeEnum.ASSET, getCurrentOrganVO().getId(), getFromSerial(), getToSerial(),
				getSanadhesabdariItemFilter(), getBeforeClosingAccounts(), getNumberOfDecimals());
	}
	...
	public List<SanadHesabdariItemVO> getIncomeGroupByHesabKol() {
		return getMyService().getTarazKolAzmayeshi(getSelectedSaalMaali(), getFromDate(), getToDate(), getHesabKolIds(),
				getMoeenIds(), getTafsiliIds(), getTafsiliTwoIds(), getAccountingMarkazIds(), HesabTypeEnum.INCOME,
				getCurrentOrganVO().getId(), getFromSerial(), getToSerial(), getSanadhesabdariItemFilter(),
				getNumberOfDecimals());
	}
	...
	public List<SanadHesabdariItemVO> getIncomeGroupByHesabMoeen() {
		return getMyService().getTarazMoeenAzmayeshi(getSelectedSaalMaali(), getFromDate(), getToDate(),
				getHesabKolIds(), getMoeenIds(), getTafsiliIds(), getTafsiliTwoIds(), getAccountingMarkazIds(),
				HesabTypeEnum.INCOME, getCurrentOrganVO().getId(), getFromSerial(), getToSerial(),
				getSanadhesabdariItemFilter(), getBeforeClosingAccounts(), getNumberOfDecimals());
	}
	...
	public List<SanadHesabdariItemVO> getExpenseGroupByHesabKol() {
		return getMyService().getTarazKolAzmayeshi(getSelectedSaalMaali(), getFromDate(), getToDate(), getHesabKolIds(),
				getMoeenIds(), getTafsiliIds(), getTafsiliTwoIds(), getAccountingMarkazIds(), HesabTypeEnum.EXPENSE,
				getCurrentOrganVO().getId(), getFromSerial(), getToSerial(), getSanadhesabdariItemFilter(),
				getNumberOfDecimals());
	}
	...
	public List<SanadHesabdariItemVO> getExpenseGroupByHesabMoeen() {
		return getMyService().getTarazMoeenAzmayeshi(getSelectedSaalMaali(), getFromDate(), getToDate(),
				getHesabKolIds(), getMoeenIds(), getTafsiliIds(), getTafsiliTwoIds(), getAccountingMarkazIds(),
				HesabTypeEnum.EXPENSE, getCurrentOrganVO().getId(), getFromSerial(), getToSerial(),
				getSanadhesabdariItemFilter(), getBeforeClosingAccounts(), getNumberOfDecimals());
	}
	...
	public String printDaftarMoeen() {
		...
		createDaftarLocalFilter(getCurrentOrganVO().getId());
		String organName = getCurrentOrganVO().getName();
	...
	private List<SanadHesabdariItemEntity> getSanadHesabdariItemListSortedByMahiat(
			List<SanadHesabdariItemEntity> itemsList) {
	...
	private List<SanadHesabdariItemEntity> getDaftarKolSummaryListSortedByMahiat(
			List<SanadHesabdariItemEntity> itemsList) {
		List<SanadHesabdariItemEntity> daftarKolListSortedByMahiat = new ArrayList<>();
	...
	private String printMonthlySummaryDaftarRooznameh(String organName) {
		...
		Collections.sort(daftarRooznamehList, new Comparator<SanadHesabdariItemEntity>() {
			public int compare(SanadHesabdariItemEntity m1, SanadHesabdariItemEntity m2) {
				int myInt = 0;
				try {
					if (m1.getSanadHesabdari().getTarikhSanad() != null
							&& m2.getSanadHesabdari().getTarikhSanad() != null)
...
//		});
		...
		try {
			...
			byte[] pdf = JasperExportManager.exportReportToPdf(jasperPrint);
			String contentType = "application/pdf";
			downloadStream(pdf, contentType,
					SerajMessageUtil.getMessage("monthlySummaryDaftarRooznameh") + "_" + organName + ".pdf");
		} catch (JRException e) {
			e.printStackTrace();
		}
...
//	
	...
	private String printDaftarAccountingMarkaz(String organName) {
		...
		try {
			...
			String contentType = "application/pdf";
			downloadStream(pdf, contentType,
					SerajMessageUtil.getMessage("daftarAccountingMarkaz") + "_" + organName + ".pdf");
		} catch (JRException e) {
			e.printStackTrace();
		}
	...
	public void exportTarazKolAzmayeshiToExcel() {
		List<String> columnsToShow = Arrays.asList("hesabKolCode", "hesabKolName", "mandehBedehkarEbtedayDore",
				"mandehBestankarEbtedayDore", "bedehkar", "bestankar", "operationSummaryBedehkar",
				"operationSummaryBestankar", "mandehBedehkar",
				"mandehBestankar"/*
									 * , "state", "confirmationDate", "pardakhtKonnadeh", "mablaghDaryaft",
									 * "mablaghPardakht", "serial"
									 */);
		...
		columnsLabel.put("mandehBedehkarEbtedayDore",
				SerajMessageUtil.getMessage("SanadHesabdariItem_mandeh_bedehkar_ebtedaye_dore"));
		columnsLabel.put("mandehBestankarEbtedayDore",
				SerajMessageUtil.getMessage("SanadHesabdariItem_mandeh_bestankar_ebtedaye_dore"));
		...
		columnsLabel.put("operationSummaryBedehkar",
				SerajMessageUtil.getMessage("SanadHesabdariItem_operationSummaryBedehkar"));
		columnsLabel.put("operationSummaryBestankar",
				SerajMessageUtil.getMessage("SanadHesabdariItem_operationSummaryBestankar"));
		...
		List<SanadHesabdariItemVO> tarazKolAzmayehiList = extractTarazKolAzmayeshi(getCurrentOrganVO().getId());
		try {
			ByteArrayOutputStream byteArrayOutputStream = getMyService()
					.exportSimpleObjectListToXLSX(tarazKolAzmayehiList, columnsToShow, columnsLabel);
			downloadStream(byteArrayOutputStream.toByteArray(),
					"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "tarazKolAzmayeshi.xlsx");
	...
	public String printLocalTarazKolAzmayeshi() {
		...
		String organName = getCurrentOrganVO().getName();
		return printTarazKolAzmayeshi(organName, getCurrentOrganVO().getId());
	}
...
//		OrganEntity selectedOrgan = getSelectedOrganId()!=null ? getOrganService().load(getSelectedOrganId()) : null;
		String organName = getHierarchicalOrganName();
		return printTarazKolAzmayeshi(organName, getSelectedOrganId());
	}

	private String printTarazKolAzmayeshi(String organName, Long organId) {
		List<SanadHesabdariItemVO> tarazKolAzmayehiList = extractTarazKolAzmayeshi(organId);
		Map<String, Object> parameters = new HashMap<String, Object>();
		parameters.put("organName", organName);
		parameters.put("creatorUser", getCurrentUserVO().getShakhsName());
		...
		parameters.put("creationDate",
				DateConverter.toShamsiDate(DateConverter.getCurrentDateTime(), SerajDateTimePickerType.DateHorMin));

		String reportPath = getLocalFilePath(
				"/WEB-INF/classes/report/taraz/tarazKolAzmayeshi-" + getColumnsCount() + "_col.jrxml");
		...
		try {
			...
			String contentType = "application/pdf";
			downloadStream(pdf, contentType,
					SerajMessageUtil.getMessage("tarazKolAzmayehi_" + getColumnsCount()) + "_" + organName + ".pdf");
		} catch (JRException e) {
			e.printStackTrace();
		}
	...
	public void logPrintTarazKolAzmayeshiAction() {
		...
		try {
			...
			ActionLogUtil.logAction(
					SerajMessageUtil.getMessage(SerajMessageUtil.getMessage("TarazKolAzmayeshi_PRINT_SANAD")), message,
					null, null, "");
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void exportTarazMoeenAzmayeshiToExcel() {
		List<String> columnsToShow = Arrays.asList("hesabKolName", "hesabMoeenCode", "hesabMoeenName",
				"mandehBedehkarEbtedayDore", "mandehBestankarEbtedayDore", "bedehkar", "bestankar",
				"operationSummaryBedehkar", "operationSummaryBestankar", "mandehBedehkar",
				"mandehBestankar"/*
									 * , "state", "confirmationDate", "pardakhtKonnadeh", "mablaghDaryaft",
									 * "mablaghPardakht", "serial"
									 */);
		...
		List<SanadHesabdariItemVO> tarazMoeenAzmayehiList = extractTarazMoeenAzmayeshi(getCurrentOrganVO().getId());
		try {
			ByteArrayOutputStream byteArrayOutputStream = getMyService()
					.exportSimpleObjectListToXLSX(tarazMoeenAzmayehiList, columnsToShow, columnsLabel);
			downloadStream(byteArrayOutputStream.toByteArray(),
					"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "tarazMoeenAzmayeshi.xlsx");
		} catch (IllegalAccessException e) {
			e.printStackTrace();
		} catch (InvocationTargetException e) {
			e.printStackTrace();
	...
	public String printLocalTarazMoeenAzmayeshi() {
		...
		String organName = getCurrentOrganVO().getName();
		return printTarazMoeenAzmayeshi(organName, getCurrentOrganVO().getId());
	}
...
//		OrganEntity selectedOrgan = getSelectedOrganId()!=null ? getOrganService().load(getSelectedOrganId()) : null;
		String organName = getHierarchicalOrganName();
		return printTarazMoeenAzmayeshi(organName, getSelectedOrganId());
	}

	private String printTarazMoeenAzmayeshi(String organName, Long organId) {
		List<SanadHesabdariItemVO> tarazMoeenAzmayehiList = extractTarazMoeenAzmayeshi(organId);
		Map<String, Object> parameters = new HashMap<String, Object>();
		parameters.put("organName", organName);
		parameters.put("creatorUser", getCurrentUserVO().getShakhsName());
		...
		for (Long kolId : ids) {
			hesabKolNames = hesabKolNames + hesabKolService.load(kolId) + " - ";
		}
		parameters.put("hesabKol", hesabKolNames);

		String reportPath = getLocalFilePath(
				"/WEB-INF/classes/report/taraz/tarazMoeenAzmayeshi-" + getColumnsCount() + "_col.jrxml");
		...
		try {
			...
			String contentType = "application/pdf";
			downloadStream(pdf, contentType,
					SerajMessageUtil.getMessage("tarazMoeenAzmayehi_" + getColumnsCount()) + "_" + organName + ".pdf");
		} catch (JRException e) {
			e.printStackTrace();
		}
	...
	public void logPrintTarazMoeenAzmayeshiAction() {
		...
		try {
			...
			ActionLogUtil.logAction(
					SerajMessageUtil.getMessage(SerajMessageUtil.getMessage("TarazMoeenAzmayeshi_PRINT_SANAD")),
					message, null, null, "");
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void exportTarazTafsiliAzmayeshiToExcel() {
		List<String> columnsToShow = Arrays.asList("hesabTafsiliCode", "hesabTafsiliName", "mandehBedehkarEbtedayDore",
				"mandehBestankarEbtedayDore", "bedehkar", "bestankar", "operationSummaryBedehkar",
				"operationSummaryBestankar", "mandehBedehkar",
				"mandehBestankar"/*
									 * , "state", "confirmationDate", "pardakhtKonnadeh", "mablaghDaryaft",
		...
		columnsLabel.put("mandehBedehkar", SerajMessageUtil.getMessage("SanadHesabdariItem_mandeh_bedehkar"));
		columnsLabel.put("mandehBestankar", SerajMessageUtil.getMessage("SanadHesabdariItem_mandeh_bestankar"));
		List<SanadHesabdariItemVO> tarazTafsiliAzmayehiList = extractTarazTafsiliAzmayeshi(getCurrentOrganVO().getId());
		try {
			ByteArrayOutputStream byteArrayOutputStream = getMyService()
					.exportSimpleObjectListToXLSX(tarazTafsiliAzmayehiList, columnsToShow, columnsLabel);
			downloadStream(byteArrayOutputStream.toByteArray(),
					"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "tarazTafsiliAzmayeshi.xlsx");
		} catch (IllegalAccessException e) {
			e.printStackTrace();
		} catch (InvocationTargetException e) {
			e.printStackTrace();
...
//	}
	public void exportTarazAccountingMarkazAzmayeshiToExcel() {
		List<String> columnsToShow = Arrays.asList("accountingMarkazCode", "accountingMarkazName",
				"mandehBedehkarEbtedayDore", "mandehBestankarEbtedayDore", "bedehkar", "bestankar",
				"operationSummaryBedehkar", "operationSummaryBestankar", "mandehBedehkar",
				"mandehBestankar"/*
									 * , "state", "confirmationDate", "pardakhtKonnadeh", "mablaghDaryaft",
...
//		OrganEntity currentOrgan = getCurrentOrgan();
		return printTarazTafsiliAzmayeshi(getCurrentOrganVO().getName(), getCurrentOrganVO().getId());
	}
...
//		OrganEntity selectedOrgan = getSelectedOrganId()!=null ? getOrganService().load(getSelectedOrganId()) : null;
		String organName = getHierarchicalOrganName();
		return printTarazTafsiliAzmayeshi(organName, getSelectedOrganId());
	}

	private String printTarazTafsiliAzmayeshi(String organName, Long organId) {
		List<SanadHesabdariItemVO> tarazTafsiliAzmayehiList = extractTarazTafsiliAzmayeshi(organId);
		Map<String, Object> parameters = populateReportParameters(organName);

		String reportPath = getLocalFilePath(
				"/WEB-INF/classes/report/taraz/tarazTafsiliAzmayeshi-" + getColumnsCount() + "_col.jrxml");
		...
		try {
			...
			byte[] pdf = JasperExportManager.exportReportToPdf(jasperPrint);
			String contentType = "application/pdf";
			downloadStream(pdf, contentType, SerajMessageUtil.getMessage("tarazTafsiliAzmayeshi_" + getColumnsCount())
					+ "_" + organName + ".pdf");
		} catch (JRException e) {
			e.printStackTrace();
		}
...
//		String organName = organEntity.getName();
		return printTarazAccountingMarkazAzmayeshi(getCurrentOrganVO().getName(), getCurrentOrganVO().getId());
	}
...
//		OrganEntity organEntity = getCurrentOrgan();
//		String organName = organEntity.getName();
		return printTarazAccountingMarkazShenavarAzmayeshi(getCurrentOrganVO().getName(), getCurrentOrganVO().getId());
	}
...
//		OrganEntity selectedOrgan = getSelectedOrganId()!=null ? getOrganService().load(getSelectedOrganId()) : null;
		String organName = getHierarchicalOrganName();
		return printTarazAccountingMarkazAzmayeshi(organName, getSelectedOrganId());
	}
...
//		OrganEntity selectedOrgan = getSelectedOrganId()!=null ? getOrganService().load(getSelectedOrganId()) : null;
		String organName = getHierarchicalOrganName();
		return printTarazAccountingMarkazShenavarAzmayeshi(organName, getSelectedOrganId());
	}

	private String printTarazAccountingMarkazAzmayeshi(String organName, Long organId) {
		List<SanadHesabdariItemVO> tarazAccountingMarkazAzmayehiList = extractTarazAccountingMarkazAzmayeshi(organId);
		Map<String, Object> parameters = populateReportParameters(organName);

		String reportPath = getLocalFilePath(
				"/WEB-INF/classes/report/taraz/tarazAccountingMarkazAzmayeshi-" + getColumnsCount() + "_col.jrxml");
		...
		try {
			...
			String contentType = "application/pdf";
			downloadStream(pdf, contentType,
					SerajMessageUtil.getMessage("tarazAccountingMarkazAzmayeshi_" + getColumnsCount()) + "_" + organName
							+ ".pdf");
		} catch (JRException e) {
			e.printStackTrace();
		}
	...
	private String printTarazAccountingMarkazShenavarAzmayeshi(String organName, Long organId) {
		List<SanadHesabdariItemEntity> tarazAccountingMarkazAzmayehiList = extractTarazAccountingMarkazShenavarAzmayeshi(
				organId);
		Map<String, Object> parameters = populateReportParameters(organName);
	...
	private Map<String, Object> populateReportParameters(String organName) {
		...
		for (Long moeenId : moeenIds) {
			hesabMoeenNames = hesabMoeenNames + hesabMoeenService.load(moeenId) + " - ";
		}
		...
		for (Long tafsiliId : tafsiliIds) {
			hesabtafsiliNames = hesabtafsiliNames + hesabTafsiliService.load(tafsiliId) + " - ";
		}
		...
		if (tafsiliTwoIds != null) {
			for (Long tafsiliTwoId : tafsiliTwoIds) {
				hesabtafsiliTwoNames = hesabtafsiliTwoNames + hesabTafsiliService.load(tafsiliTwoId) + " - ";
			}
		}
...
//				hesabtafsiliShenavarNames=hesabtafsiliShenavarNames+hesabTafsiliService.load(tafsiliId)+" - ";
//			}
...
//		}
	...
	public void logPrintTarazTafsiliAzmayeshiAction() {
		...
		try {
			...
			ActionLogUtil.logAction(
					SerajMessageUtil.getMessage(SerajMessageUtil.getMessage("TarazTafsiliAzmayeshi_PRINT_SANAD")),
					message, null, null, "");
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String printTarazKol() {
		...
		Map<String, Object> parameters = new HashMap<String, Object>();
		parameters.put("organName", getCurrentOrganVO().getName());
		...
		try {
			jasperReport = JasperCompileManager.compileReport(reportPath);
			JasperReport jasperReport_darayi = JasperCompileManager
					.compileReport(getLocalFilePath("/WEB-INF/classes/report/tarazKol_subreport_darayi.jrxml"));
			parameters.put("tarazKolDarayiJasperReport", jasperReport_darayi);
			JasperReport jasperReport_bedehi = JasperCompileManager
					.compileReport(getLocalFilePath("/WEB-INF/classes/report/tarazKol_subreport_bedehi.jrxml"));
			parameters.put("tarazKolBedehiJasperReport", jasperReport_bedehi);
			JasperReport jasperReport_sarmayeh = JasperCompileManager
					.compileReport(getLocalFilePath("/WEB-INF/classes/report/tarazKol_subreport_sarmaye.jrxml"));
			...
			String contentType = "application/pdf";
			downloadStream(pdf, contentType,
					SerajMessageUtil.getMessage("tarazKol") + "_" + getCurrentOrganVO().getName() + ".pdf");
		} catch (JRException e) {
			e.printStackTrace();
		}
	...
	public String printTarazMoeen() {
		...
		try {
			jasperReport = JasperCompileManager.compileReport(reportPath);
			JasperReport jasperReport_darayi = JasperCompileManager
					.compileReport(getLocalFilePath("/WEB-INF/classes/report/tarazMoeen_subreport_darayi.jrxml"));
			parameters.put("tarazMoeenDarayiJasperReport", jasperReport_darayi);
			JasperReport jasperReport_bedehi = JasperCompileManager
					.compileReport(getLocalFilePath("/WEB-INF/classes/report/tarazMoeen_subreport_bedehi.jrxml"));
			parameters.put("tarazMoeenBedehiJasperReport", jasperReport_bedehi);
			JasperReport jasperReport_sarmayeh = JasperCompileManager
					.compileReport(getLocalFilePath("/WEB-INF/classes/report/tarazMoeen_subreport_sarmaye.jrxml"));
			...
			String contentType = "application/pdf";
			downloadStream(pdf, contentType,
					SerajMessageUtil.getMessage("tarazMoeen") + "_" + getCurrentOrganVO().getName() + ".pdf");
		} catch (JRException e) {
			e.printStackTrace();
		}
	...
	public String printTarazTafsili() {
		...
		try {
			jasperReport = JasperCompileManager.compileReport(reportPath);
			JasperReport jasperReport_darayi = JasperCompileManager
					.compileReport(getLocalFilePath("/WEB-INF/classes/report/tarazTafsili_subreport_darayi.jrxml"));
			parameters.put("tarazTafsiliDarayiJasperReport", jasperReport_darayi);
			JasperReport jasperReport_bedehi = JasperCompileManager
					.compileReport(getLocalFilePath("/WEB-INF/classes/report/tarazTafsili_subreport_bedehi.jrxml"));
			parameters.put("tarazTafsiliBedehiJasperReport", jasperReport_bedehi);
			JasperReport jasperReport_sarmayeh = JasperCompileManager
					.compileReport(getLocalFilePath("/WEB-INF/classes/report/tarazTafsili_subreport_sarmaye.jrxml"));
			...
			String contentType = "application/pdf";
			downloadStream(pdf, contentType,
					SerajMessageUtil.getMessage("tarazTafsili") + "_" + getCurrentOrganVO().getName() + ".pdf");
		} catch (JRException e) {
			e.printStackTrace();
		}
	...
	public void logPrintTarazKolAction() {
		...
		try {
			...
			ActionLogUtil.logAction(SerajMessageUtil.getMessage(SerajMessageUtil.getMessage("TarazKol_PRINT_SANAD")),
					message, null, null, "");
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void logPrintTarazMoeenAction() {
		...
		try {
			...
			ActionLogUtil.logAction(SerajMessageUtil.getMessage(SerajMessageUtil.getMessage("TarazMoeen_PRINT_SANAD")),
					message, null, null, "");
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void logPrintTarazTafsiliAction() {
		...
		try {
			...
			ActionLogUtil.logAction(
					SerajMessageUtil.getMessage(SerajMessageUtil.getMessage("TarazTafsili_PRINT_SANAD")), message, null,
					null, "");
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String goTodDftarRooznamehHierarchicalList() {
		return getFacesUrl(
				"/sanadhesabdariitem/daftar/hierarchical/sanadHesabdariItem.daftarRooznamehHierarchicalList.xhtml");
	}
	...
	public void populateDaftarKolJsonList() {
		...
		try {
			...
			for (SanadHesabdariItemEntity sanadHesabdariItemEntity : list) {
				...
				tashkhis = mandeh > 0 ? SerajMessageUtil.getMessage("SanadHesabdari_bed")
						: SerajMessageUtil.getMessage("SanadHesabdari_bes");
				String[] nextLine = { Integer.valueOf(++index).toString(),
						sanadHesabdariItemEntity.getSanadHesabdari().getId().toString(),
						sanadHesabdariItemEntity.getTempSanadSerial(), sanadHesabdariItemEntity.getSanadSerial(),
						sanadHesabdariItemEntity.getSanadHesabdari().getTarikhSanadFA(),
						sanadHesabdariItemEntity.getHesabKol().getName(), sanadHesabdariItemEntity.getHesabKolCode(),
						sanadHesabdariItemEntity.getDescription(),
						NumberUtil.getBigDecimalFormatted(sanadHesabdariItemEntity.getBedehkar(),
								getNumberOfDecimals()),
						NumberUtil.getBigDecimalFormatted(sanadHesabdariItemEntity.getBestankar(),
								getNumberOfDecimals()),
						NumberUtil.getBigDecimalFormatted(mandeh, getNumberOfDecimals()), tashkhis };
