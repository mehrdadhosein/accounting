...
//	}
	...
	public List<SanadHesabdariItemVO> getTarazKolAzmayeshi(SaalMaaliEntity saalMaaliEntity, Date fromDate, Date toDate, List<Long> hesabKolIds, List<Long> moeenIds,List<Long> tafsiliIds, List<Long> articleTafsiliIds, List<Long> accountingMarkazIds, HesabTypeEnum hesabType, Long organId, Long fromSerial, Long toSerial, Map<String, Object> sanadhesabdariItemFilter, int numberOfDecimals) {
		return getTarazKolAzmayeshi(saalMaaliEntity, fromDate, toDate, hesabKolIds, moeenIds, tafsiliIds, articleTafsiliIds, accountingMarkazIds, hesabType, organId, fromSerial, toSerial, sanadhesabdariItemFilter, true, true, numberOfDecimals);
	}
	
	@Transactional(readOnly=true)
	public List<SanadHesabdariItemVO> getTarazKolAzmayeshi(SaalMaaliEntity saalMaaliEntity, Date fromDate, Date toDate, List<Long> hesabKolIds, List<Long> moeenIds,List<Long> tafsiliIds, List<Long> articleTafsiliIds, List<Long> accountingMarkazIds, HesabTypeEnum hesabType, Long organId, Long fromSerial, Long toSerial, Map<String, Object> sanadhesabdariItemFilter, boolean displayZeroMandehInTaraz, boolean beforeClosingAccounts, int numberOfDecimals) {
		List<SanadHesabdariItemVO> tarazKolAzmayeshiList = new ArrayList<SanadHesabdariItemVO>();
		Map<String, Object> localFilter = new HashMap<String, Object>();
		localFilter = populateTarazFilter(saalMaaliEntity, fromDate, toDate, hesabKolIds, moeenIds, tafsiliIds, articleTafsiliIds, accountingMarkazIds, hesabType, organId, fromSerial, toSerial, sanadhesabdariItemFilter, beforeClosingAccounts);
		
		List<Object[]> rawList = getMyDAO().getTarazKolAzmayeshi(localFilter);
		Map<Long, SanadHesabdariItemVO> tarazKolAzmayeshiMandeh = getTarazKolAzmayeshiMandeh(saalMaaliEntity, fromDate, hesabKolIds, moeenIds, tafsiliIds, articleTafsiliIds, accountingMarkazIds, hesabType, organId, beforeClosingAccounts, numberOfDecimals);
	...
	@Transactional(readOnly=true)
	private Map<Long, SanadHesabdariItemVO> getTarazKolAzmayeshiMandeh(SaalMaaliEntity saalMaaliEntity, Date fromDate, List<Long> hesabKolIds, List<Long> moeenIds, List<Long> tafsiliIds, List<Long> articleTafsiliIds, List<Long> accountingMarkazIds, HesabTypeEnum hesabType, Long organId, boolean beforeClosingAccounts, int numberOfDecimals) {
		Map<Long, SanadHesabdariItemVO> tarazKolAzmayeshiMap = new HashMap<Long, SanadHesabdariItemVO>();
		Map<String, Object> localFilter = new HashMap<String, Object>();
		localFilter = populateMandehTarazFilter(saalMaaliEntity, fromDate, hesabKolIds, moeenIds, tafsiliIds, articleTafsiliIds, accountingMarkazIds, hesabType, organId, beforeClosingAccounts);
		...
		if(organId == null)
			...
			applySanadEftetahiehOnTarazKolAzmayeshiMandeh(tarazKolAzmayeshiMap, saalMaaliEntity, organId, numberOfDecimals);
		return tarazKolAzmayeshiMap;
	}
	
	@Transactional(readOnly=true)
	private void applySanadEftetahiehOnTarazKolAzmayeshiMandeh(Map<Long, SanadHesabdariItemVO> tarazKolAzmayeshiMap, SaalMaaliEntity saalMaaliEntity, int numberOfDecimals) {
	...
	@Transactional(readOnly=true)
	private void applySanadEftetahiehOnTarazKolAzmayeshiMandeh(Map<Long, SanadHesabdariItemVO> tarazKolAzmayeshiMap, SaalMaaliEntity saalMaaliEntity, Long organId, int numberOfDecimals) {
	...
	private void applySanadEftetahiehOnTarazKolAzmayeshiMandeh(
		...
		for (SanadHesabdariItemEntity sanadHesabdariItemEntity : sanadHesabdariItems) {
			...
			if(itemEntity == null){
				...
				tarazKolAzmayeshiMap.put(hesabKolId, new SanadHesabdariItemVO(sanadHesabdariItemEntity, numberOfDecimals));
			}
			else{
	...
	@Transactional(readOnly=true)
	private void applySanadEftetahiehOnTarazMoeenAzmayeshiMandeh(Map<Long, SanadHesabdariItemVO> tarazMoeenAzmayeshiMap, SaalMaaliEntity saalMaaliEntity, int numberOfDecimals) {
		List<SanadHesabdariEntity> sanadEftetahiehha = getSanadHesabdariService().getSanadEftetahiehha(saalMaaliEntity);
		for (SanadHesabdariEntity sanadEftetahieh : sanadEftetahiehha) {
	...
	@Transactional(readOnly=true)
	private void applySanadEftetahiehOnTarazMoeenAzmayeshiMandeh(Map<Long, SanadHesabdariItemVO> tarazMoeenAzmayeshiMap, SaalMaaliEntity saalMaaliEntity, Long organId, int numberOfDecimals) {
		SanadHesabdariEntity sanadEftetahieh = getSanadHesabdariService().getSanadEftetahieh(saalMaaliEntity,organId);
		if(sanadEftetahieh == null || sanadEftetahieh.getSanadHesabdariItem() == null)
			return;
		applySanadEftetahiehOnTarazMoeenAzmayeshiMandeh(tarazMoeenAzmayeshiMap, sanadEftetahieh, numberOfDecimals);
...
//				sanadHesabdariItemEntity.setMandehBestankar(0d);
				tarazMoeenAzmayeshiMap.put(hesabMoeenId, new SanadHesabdariItemVO(sanadHesabdariItemEntity, numberOfDecimals));
			}
			else{
				itemEntity.setBestankar(itemEntity.getBestankar()+sanadHesabdariItemEntity.getBestankar());
				itemEntity.setBedehkar(itemEntity.getBedehkar()+sanadHesabdariItemEntity.getBedehkar());
	...
	@Transactional(readOnly=true)
	private void applySanadEftetahiehOnTarazTafsiliAzmayeshiMandeh(Map<Long, SanadHesabdariItemVO> tarazTafsiliAzmayeshiMap, SaalMaaliEntity saalMaaliEntity, int numberOfDecimals) {
		List<SanadHesabdariEntity> sanadEftetahiehha = getSanadHesabdariService().getSanadEftetahiehha(saalMaaliEntity);
		for (SanadHesabdariEntity sanadEftetahieh : sanadEftetahiehha) {
	...
	@Transactional(readOnly=true)
	private void applySanadEftetahiehOnTarazTafsiliAzmayeshiMandeh(Map<Long, SanadHesabdariItemVO> tarazTafsiliAzmayeshiMap, SaalMaaliEntity saalMaaliEntity, Long organId, int numberOfDecimals) {
		SanadHesabdariEntity sanadEftetahieh = getSanadHesabdariService().getSanadEftetahieh(saalMaaliEntity,organId);
		if(sanadEftetahieh == null || sanadEftetahieh.getSanadHesabdariItem() == null)
			return;
		applySanadEftetahiehOnTarazTafsiliAzmayeshiMandeh(tarazTafsiliAzmayeshiMap, sanadEftetahieh, numberOfDecimals);
...
//				sanadHesabdariItemEntity.setMandehBestankar(0d);
				...
				tarazTafsiliAzmayeshiMap.put(hesabTafsiliId, new SanadHesabdariItemVO(sanadHesabdariItemEntity, numberOfDecimals));
			}
			else{
				itemEntity.setBestankar(itemEntity.getBestankar()+sanadHesabdariItemEntity.getBestankar());
				itemEntity.setBedehkar(itemEntity.getBedehkar()+sanadHesabdariItemEntity.getBedehkar());
	...
	@Transactional(readOnly=true)
	private void applySanadEftetahiehOnTarazMarkazAzmayeshiMandeh(Map<Long, SanadHesabdariItemVO> tarazMarkazAzmayeshiMap, SaalMaaliEntity saalMaaliEntity, int numberOfDecimals) {
		List<SanadHesabdariEntity> sanadEftetahiehha = getSanadHesabdariService().getSanadEftetahiehha(saalMaaliEntity);
		for (SanadHesabdariEntity sanadEftetahieh : sanadEftetahiehha) {
	...
	@Transactional(readOnly=true)
	private void applySanadEftetahiehOnTarazMarkazAzmayeshiMandeh(Map<Long, SanadHesabdariItemVO> tarazMarkazAzmayeshiMap, SaalMaaliEntity saalMaaliEntity, Long organId, int numberOfDecimals) {
		SanadHesabdariEntity sanadEftetahieh = getSanadHesabdariService().getSanadEftetahieh(saalMaaliEntity,organId);
		if(sanadEftetahieh == null || sanadEftetahieh.getSanadHesabdariItem() == null)
			return;
		applySanadEftetahiehOnTarazMarkazAzmayeshiMandeh(tarazMarkazAzmayeshiMap, sanadEftetahieh, numberOfDecimals);
	...
	private void applySanadEftetahiehOnTarazMarkazAzmayeshiMandeh(
		...
		for (SanadHesabdariItemEntity sanadHesabdariItemEntity : sanadHesabdariItems) {
			...
			if(itemEntity == null)
				tarazMarkazAzmayeshiMap.put(accountingMarkazId, new SanadHesabdariItemVO(sanadHesabdariItemEntity, numberOfDecimals));
			else{
				itemEntity.setBestankar(itemEntity.getBestankar()+sanadHesabdariItemEntity.getBestankar());
				itemEntity.setBedehkar(itemEntity.getBedehkar()+sanadHesabdariItemEntity.getBedehkar());
			}
	...
	@Transactional(readOnly=true)
	private Map<Long, SanadHesabdariItemVO> getTarazMoeenAzmayeshiMandeh(SaalMaaliEntity saalMaaliEntity, Date fromDate, List<Long> hesabKolIds, List<Long> moeenIds, List<Long> tafsiliIds, List<Long> articleTafsiliIds, List<Long> accountingMarkazIds, HesabTypeEnum hesabType, Long organId, boolean beforeClosingAccounts, int numberOfDecimals) {
		...
		if(organId == null)
			...
			applySanadEftetahiehOnTarazMoeenAzmayeshiMandeh(tarazMoeenAzmayeshiMap, saalMaaliEntity, organId, numberOfDecimals);
	...
	@Transactional(readOnly=true)
	private Map<Long, SanadHesabdariItemVO> getTarazTafsiliAzmayeshiMandeh(SaalMaaliEntity saalMaaliEntity, Date fromDate, List<Long> hesabKolIds, List<Long> moeenIds, List<Long> tafsiliIds, List<Long> articleTafsiliIds, List<Long> accountingMarkazIds, HesabTypeEnum hesabType, Long organId, boolean beforeClosingAccounts, int numberOfDecimals) {
		...
		if(organId == null)
			applySanadEftetahiehOnTarazTafsiliAzmayeshiMandeh(tarazTafsiliAzmayeshiMap, saalMaaliEntity, numberOfDecimals);
		else
			applySanadEftetahiehOnTarazTafsiliAzmayeshiMandeh(tarazTafsiliAzmayeshiMap, saalMaaliEntity, organId, numberOfDecimals);
		
		return tarazTafsiliAzmayeshiMap;
	}
	@Transactional(readOnly=true)
	private Map<Long, SanadHesabdariItemVO> getTarazTafsiliAzmayeshiTwoMandeh(SaalMaaliEntity saalMaaliEntity, Date fromDate, List<Long> hesabKolIds, List<Long> moeenIds, List<Long> tafsiliIds, List<Long> articleTafsiliIds, List<Long> accountingMarkazIds, HesabTypeEnum hesabType, Long organId, boolean beforeClosingAccounts, int numberOfDecimals) {
		Map<Long, SanadHesabdariItemVO> tarazTafsiliAzmayeshiMap = new HashMap<Long, SanadHesabdariItemVO>();
		Map<String, Object> localFilter = new HashMap<String, Object>();
		localFilter = populateMandehTarazFilter(saalMaaliEntity, fromDate, hesabKolIds, moeenIds, tafsiliIds, articleTafsiliIds, accountingMarkazIds, hesabType, organId, beforeClosingAccounts);
	...
	@Transactional(readOnly=true)
	private Map<Long, SanadHesabdariItemVO> getTarazTafsiliShenavarAzmayeshiMandeh(SaalMaaliEntity saalMaaliEntity, Date fromDate, List<Long> hesabKolIds, List<Long> moeenIds, List<Long> tafsiliIds, List<Long> articleTafsiliIds, List<Long> accountingMarkazIds, HesabTypeEnum hesabType, Long organId, Integer level, boolean beforeClosingAccounts, int numberOfDecimals) {
		Map<Long, SanadHesabdariItemVO> tarazTafsiliAzmayeshiMap = new HashMap<Long, SanadHesabdariItemVO>();
		Map<String, Object> localFilter = new HashMap<String, Object>();
		localFilter = populateMandehTarazFilter(saalMaaliEntity, fromDate, hesabKolIds, moeenIds, tafsiliIds, articleTafsiliIds, accountingMarkazIds, hesabType, organId, beforeClosingAccounts);
...
//		localFilter.put("articleTafsili.level@eq", 2);
		...
		if(organId == null)
			applySanadEftetahiehOnTarazMarkazAzmayeshiMandeh(tarazAccountingMarkazAzmayeshiMap, saalMaaliEntity, numberOfDecimals);
		else
			applySanadEftetahiehOnTarazMarkazAzmayeshiMandeh(tarazAccountingMarkazAzmayeshiMap, saalMaaliEntity, organId, numberOfDecimals);
		
		return tarazAccountingMarkazAzmayeshiMap;
	}
	
	@Transactional(readOnly=true)
	public List<SanadHesabdariItemVO> getTarazMoeenAzmayeshi(SaalMaaliEntity saalMaaliEntity, Date fromDate, Date toDate, List<Long> hesabKolIds, List<Long> moeenIds,List<Long> tafsiliIds, List<Long> articleTafsiliIds, List<Long> accountingMarkazIds, HesabTypeEnum hesabType, Long organId, Long fromSerial, Long toSerial, Map<String, Object> sanadhesabdariItemFilter, boolean beforeClosingAccounts, int numberOfDecimals) {
		return getTarazMoeenAzmayeshi(saalMaaliEntity, fromDate, toDate, hesabKolIds, moeenIds, tafsiliIds, articleTafsiliIds, accountingMarkazIds, hesabType, organId, fromSerial, toSerial, sanadhesabdariItemFilter, true, beforeClosingAccounts, numberOfDecimals);
	}
	...
	public List<SanadHesabdariItemVO> getTarazMoeenAzmayeshi(SaalMaaliEntity saalMaaliEntity, Date fromDate, Date toDate, List<Long> hesabKolIds, List<Long> moeenIds,List<Long> tafsiliIds, List<Long> articleTafsiliIds, List<Long> accountingMarkazIds, HesabTypeEnum hesabType, Long organId, Long fromSerial, Long toSerial, Map<String, Object> sanadhesabdariItemFilter, boolean displayZeroMandehInTaraz, boolean beforeClosingAccounts, int numberOfDecimals) {
		...
		List<Object[]> rawList = getMyDAO().getTarazMoeenAzmayeshi(localFilter);
		Map<Long, SanadHesabdariItemVO> tarazMoeenAzmayeshiMandeh = getTarazMoeenAzmayeshiMandeh(saalMaaliEntity, fromDate, hesabKolIds, moeenIds, tafsiliIds, articleTafsiliIds, accountingMarkazIds, hesabType, organId, beforeClosingAccounts, numberOfDecimals);
		for (Object object : rawList) {
			SanadHesabdariItemVO entity = new SanadHesabdariItemVO();
			Object[] a = (Object[])object;
			Long hesabMoeenId = new Long(a[0].toString());
	...
	@Transactional(readOnly=true)
	public List<SanadHesabdariItemVO> getTarazTafsiliAzmayeshiTwo(SaalMaaliEntity saalMaaliEntity, Date fromDate, Date toDate,List<Long> hesabKolIds, List<Long> moeenIds, List<Long> tafsiliIds, List<Long> articleTafsiliIds, List<Long> accountingMarkazIds, HesabTypeEnum hesabType, Long organId, Long fromSerial, Long toSerial, Map<String, Object> sanadhesabdariItemFilter, boolean beforeClosingAccounts, int numberOfDecimals) {
		return getTarazTafsiliTwoAzmayeshi(saalMaaliEntity, fromDate, toDate, hesabKolIds, moeenIds, tafsiliIds, articleTafsiliIds, accountingMarkazIds, hesabType, organId, fromSerial, toSerial, sanadhesabdariItemFilter, true, beforeClosingAccounts, numberOfDecimals);
	}
	@Transactional(readOnly=true)
	public List<SanadHesabdariItemVO> getTarazTafsiliAzmayeshi(SaalMaaliEntity saalMaaliEntity, Date fromDate, Date toDate,List<Long> hesabKolIds, List<Long> moeenIds, List<Long> tafsiliIds, List<Long> articleTafsiliIds, List<Long> accountingMarkazIds, HesabTypeEnum hesabType, Long organId, Long fromSerial, Long toSerial, Map<String, Object> sanadhesabdariItemFilter, boolean beforeClosingAccounts, int numberOfDecimals) {
		return getTarazTafsiliAzmayeshi(saalMaaliEntity, fromDate, toDate, hesabKolIds, moeenIds, tafsiliIds, articleTafsiliIds, accountingMarkazIds, hesabType, organId, fromSerial, toSerial, sanadhesabdariItemFilter, true, beforeClosingAccounts, numberOfDecimals);
	}
	@Transactional(readOnly=true)
	public List<SanadHesabdariItemVO> getTarazTafsiliAzmayeshi(SaalMaaliEntity saalMaaliEntity, Date fromDate, Date toDate,List<Long> hesabKolIds, List<Long> moeenIds, List<Long> tafsiliIds, List<Long> articleTafsiliIds, List<Long> accountingMarkazIds, HesabTypeEnum hesabType, Long organId, Long fromSerial, Long toSerial, Map<String, Object> sanadhesabdariItemFilter, boolean displayZeroMandehInTaraz, boolean beforeClosingAccounts, int numberOfDecimals) {
		...
		Map<String, Object> filter = populateTarazFilter(saalMaaliEntity,
				...
				articleTafsiliIds,accountingMarkazIds, hesabType, organId, fromSerial, toSerial, sanadhesabdariItemFilter, beforeClosingAccounts);
		

		
		List<Object[]> rawList = getMyDAO().getTarazTafsiliAzmayeshi(filter);
		Map<Long, SanadHesabdariItemVO> tarazTafsiliAzmayeshiMandeh = getTarazTafsiliAzmayeshiMandeh(saalMaaliEntity, fromDate, hesabKolIds, moeenIds, tafsiliIds, articleTafsiliIds, accountingMarkazIds, hesabType, organId, beforeClosingAccounts, numberOfDecimals);
	...
	@Transactional(readOnly=true)
	public List<SanadHesabdariItemVO> getTarazTafsiliTwoAzmayeshi(SaalMaaliEntity saalMaaliEntity, Date fromDate, Date toDate,List<Long> hesabKolIds, List<Long> moeenIds, List<Long> tafsiliIds, List<Long> articleTafsiliIds, List<Long> accountingMarkazIds, HesabTypeEnum hesabType, Long organId, Long fromSerial, Long toSerial, Map<String, Object> sanadhesabdariItemFilter, boolean displayZeroMandehInTaraz, boolean beforeClosingAccounts, int numberOfDecimals) {
		List<SanadHesabdariItemVO> tarazTafsiliAzmayeshiList = new ArrayList<SanadHesabdariItemVO>();
		Map<String, Object> filter = populateTarazFilter(saalMaaliEntity,
				fromDate, toDate, hesabKolIds, moeenIds, tafsiliIds,
				articleTafsiliIds,accountingMarkazIds, hesabType, organId, fromSerial, toSerial, sanadhesabdariItemFilter, beforeClosingAccounts);
		...
		Map<Long, SanadHesabdariItemVO> tarazTafsiliAzmayeshiTwoMandeh = getTarazTafsiliAzmayeshiTwoMandeh(saalMaaliEntity, fromDate, hesabKolIds, moeenIds, tafsiliIds, articleTafsiliIds, accountingMarkazIds, hesabType, organId, beforeClosingAccounts, numberOfDecimals);
		...
		for (Object[] object : rawList) {
			...
			if(tarazTafsiliAzmayeshiTwoMandeh.get(hesabTafsiliId)!=null){
				entity.setMandehBedehkarEbtedayDore(tarazTafsiliAzmayeshiTwoMandeh.get(hesabTafsiliId).getMandehBedehkar());
				entity.setMandehBestankarEbtedayDore(tarazTafsiliAzmayeshiTwoMandeh.get(hesabTafsiliId).getMandehBestankar());
				entity.setOperationSummaryBedehkar(entity.getMandehBedehkarEbtedayDore()+entity.getBedehkar());
				entity.setOperationSummaryBestankar(entity.getMandehBestankarEbtedayDore()+entity.getBestankar());
			}else{
				entity.setMandehBedehkarEbtedayDore(0d);
	...
	@Transactional(readOnly=true)
	public List<SanadHesabdariItemVO> getTarazTafsiliShenavarAzmayeshi(SaalMaaliEntity saalMaaliEntity, Date fromDate, Date toDate,List<Long> hesabKolIds, List<Long> moeenIds, List<Long> tafsiliIds, List<Long> articleTafsiliIds, List<Long> accountingMarkazIds, HesabTypeEnum hesabType, Long organId, Long fromSerial, Long toSerial, Map<String, Object> sanadhesabdariItemFilter, Integer level, boolean displayZeroMandehInTaraz, boolean beforeClosingAccounts, int numberOfDecimals) {
		List<SanadHesabdariItemVO> tarazTafsiliAzmayeshiList = new ArrayList<SanadHesabdariItemVO>();
		Map<String, Object> filter = populateTarazFilter(saalMaaliEntity,
				fromDate, toDate, hesabKolIds, moeenIds, tafsiliIds,
				articleTafsiliIds,accountingMarkazIds, hesabType, organId, fromSerial, toSerial, sanadhesabdariItemFilter, beforeClosingAccounts);
		...
		Map<Long, SanadHesabdariItemVO> tarazTafsiliAzmayeshiMandeh = getTarazTafsiliShenavarAzmayeshiMandeh(saalMaaliEntity, fromDate, hesabKolIds, moeenIds, tafsiliIds, articleTafsiliIds, accountingMarkazIds, hesabType, organId, level, beforeClosingAccounts, numberOfDecimals);
	...
	public List<SanadHesabdariItemVO> getTarazAccountingMarkazAzmayeshi(SaalMaaliEntity saalMaaliEntity, Date fromDate, Date toDate,List<Long> hesabKolIds, List<Long> moeenIds, List<Long> tafsiliIds, List<Long> articleTafsiliIds, List<Long> accountingMarkazIds, HesabTypeEnum hesabType, Long organId, Long fromSerial, Long toSerial, Map<String, Object> sanadhesabdariItemFilter, boolean displayZeroMandehInTaraz, boolean beforeClosingAccounts, int numberOfDecimals) {
		...
		Map<Long, SanadHesabdariItemVO> tarazAccountingMarkazAzmayeshiMandeh = getTarazAccountingMarkazAzmayeshiMandeh(saalMaaliEntity, fromDate, hesabKolIds, moeenIds, tafsiliIds, articleTafsiliIds, accountingMarkazIds, hesabType, organId, beforeClosingAccounts, numberOfDecimals);
	...
	private Map<String, Object> populateTarazFilter(
			...
			Long organId, Long fromSerial, Long toSerial, Map<String, Object> sanadhesabdariItemFilter, boolean beforeClosingAccounts) {
		Map<String, Object> localFilter =  new HashMap<String, Object>();
	...
	private Map<String, Object> populateMandehTarazFilter(
			...
			List<Long> hesabKolIds, List<Long> moeenIds, List<Long> tafsiliIds,
			List<Long> articleTafsiliIds, List<Long> accountingMarkazIds, HesabTypeEnum hesabType,
			Long organId, boolean beforeClosingAccounts) {
		Map<String, Object> localFilter =  new HashMap<String, Object>();
