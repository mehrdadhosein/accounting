...
public class HesabRelationsUtil {
	...
	public static Map<Long, List<ListOrderedMap<String, Object>>> getTafsiliMoeenMap(SaalMaaliEntity saalMaaliEntity,
			Long organId, List<Long> topOrganList) {
		Map<Long, List<ListOrderedMap<String, Object>>> tafsiliMoeenMap = organizationalTafsiliMoeenMap
				.get(saalMaaliEntity.getId() + "_" + organId);
		if (tafsiliMoeenMap == null) {
			tafsiliMoeenMap = hesabTafsiliService.getTafsiliMoeenMap(saalMaaliEntity, topOrganList);
	...
	public static Map<Long, List<ListOrderedMap<String, Object>>> getKolMoeenMap(SaalMaaliEntity saalMaaliEntity,
			Long organId, List<Long> topOrganList) {
		Map<Long, List<ListOrderedMap<String, Object>>> kolMoeenMap = organizationalKolMoeenMap
				.get(saalMaaliEntity.getId() + "_" + organId);
		if (kolMoeenMap == null) {
			kolMoeenMap = new HashMap<Long, List<ListOrderedMap<String, Object>>>();

			List<HesabKolEntity> list = hesabKolService.getHesabKolList(saalMaaliEntity, topOrganList);
			for (HesabKolEntity hesabKolEntity : list) {
				List<HesabMoeenEntity> moeenList = hesabMoeenService.getActiveMoeens(hesabKolEntity.getId(),
						saalMaaliEntity, topOrganList);
	...
	public static Map<Long, ListOrderedMap<String, Object>> getMoeenKolMap(SaalMaaliEntity saalMaaliEntity,
			Long organId, List<Long> topOrganList) {
		Map<Long, ListOrderedMap<String, Object>> moeenKolMap = organizationalMoeenKolMap
				.get(saalMaaliEntity.getId() + "_" + organId);

		if (moeenKolMap == null) {
			moeenKolMap = new HashMap<Long, ListOrderedMap<String, Object>>();

			List<HesabMoeenEntity> list = hesabMoeenService.getActiveMoeens(saalMaaliEntity, topOrganList);
			for (HesabMoeenEntity hesabMoeenEntity : list) {
				ListOrderedMap<String, Object> moeenItemMap = new ListOrderedMap<String, Object>();
				moeenItemMap.put("value",
						hesabMoeenEntity.getHesabKol() != null ? hesabMoeenEntity.getHesabKol().getID()
								: SerajMessageUtil.getMessage("common_undefined"));
				moeenItemMap.put("label",
						hesabMoeenEntity.getHesabKol() != null ? hesabMoeenEntity.getHesabKol().getDesc()
								: SerajMessageUtil.getMessage("common_undefined"));
	...
	public static Map<Long, List<ListOrderedMap<String, Object>>> getMoeenTafsiliOneMap(SaalMaaliEntity saalMaaliEntity,
			Long organId, List<Long> topOrganList) {

		Map<Long, List<ListOrderedMap<String, Object>>> moeenTafsiliOneMap = organizationalMoeenTafsiliOneMap
				.get(saalMaaliEntity.getId() + "_" + organId);

		if (moeenTafsiliOneMap == null) {
			List<HesabMoeenEntity> moeenList = hesabMoeenService.getActiveMoeens(saalMaaliEntity, topOrganList);
			...
			for (HesabMoeenEntity hesabMoeenEntity : moeenList) {
				...
				List<HesabTafsiliEntity> activeTafsilies = hesabMoeenService.getActiveTafsilies(hesabMoeenEntity,
						saalMaaliEntity, topOrganList, 1);
	...
	public static Map<Long, List<ListOrderedMap<String, Object>>> getMoeenTafsiliTwoMap(SaalMaaliEntity saalMaaliEntity,
			Long organId, List<Long> topOrganList) {

		Map<Long, List<ListOrderedMap<String, Object>>> moeenTafsiliTwoMap = organizationalMoeenTafsiliTwoMap
				.get(saalMaaliEntity.getId() + "_" + organId);
		...
		if (moeenTafsiliTwoMap == null) {
			...
			for (HesabMoeenEntity hesabMoeenEntity : moeenList) {
				List<ListOrderedMap<String, Object>> hesabTafsiliList = new ArrayList<ListOrderedMap<String, Object>>();
				List<HesabTafsiliEntity> activeTafsilies = hesabMoeenService.getActiveTafsilies(hesabMoeenEntity,
						saalMaaliEntity, topOrganList, 2);
				for (HesabTafsiliEntity tafsiliEntity : activeTafsilies) {
					if (tafsiliEntity.getHidden().equals(false)) {
						ListOrderedMap<String, Object> bankBranchItemMap = new ListOrderedMap<String, Object>();
						bankBranchItemMap.put("value", tafsiliEntity.getID());
	...
	public static Map<Long, List<ListOrderedMap<String, Object>>> getAccountingMarkazMap(
			SaalMaaliEntity saalMaaliEntity, Long organId, List<Long> topOrganList) {

		Map<Long, List<ListOrderedMap<String, Object>>> moeenAccountingMarkazMap = organizationalMoeenAccountingMarkazMap
				.get(saalMaaliEntity.getId() + "_" + organId);
		...
		if (moeenAccountingMarkazMap == null) {
			...
			organizationalMoeenAccountingMarkazMap.put(saalMaaliEntity.getId() + "_" + organId,
					moeenAccountingMarkazMap);
		}
	...
	public static Map<Long, List<ListOrderedMap<String, Object>>> getTafsiliChildMap(SaalMaaliEntity saalMaaliEntity,
			Long organId, List<Long> topOrganList) {

		Map<Long, List<ListOrderedMap<String, Object>>> tafsiliChildMap = organizationalTafsiliChildMap
				.get(saalMaaliEntity.getId() + "_" + organId);
		if (tafsiliChildMap == null) {
			tafsiliChildMap = new HashMap<Long, List<ListOrderedMap<String, Object>>>();
			List<HesabTafsiliEntity> tafsiliList = hesabTafsiliService.getActiveTafsilis(saalMaaliEntity, topOrganList);
	...
	public static Map<Long, List<ListOrderedMap<String, Object>>> getTafsiliAccountingMarkazChildMap(
			SaalMaaliEntity saalMaaliEntity, Long organId, List<Long> topOrganList) {

		Map<Long, List<ListOrderedMap<String, Object>>> tafsiliMap = organizationalTafsiliMap
				.get(saalMaaliEntity.getId() + "_" + organId);
		if (tafsiliMap == null) {
	...
	public static Map<Long, List<ListOrderedMap<String, Object>>> getAccountingMarkazChildMap(
			SaalMaaliEntity saalMaaliEntity, Long organId, List<Long> topOrganList) {

		Map<Long, List<ListOrderedMap<String, Object>>> accountingMarkazChildMap = organizationalAccountingMarkazChildMap
				.get(saalMaaliEntity.getId() + "_" + organId);
		if (accountingMarkazChildMap == null) {
			accountingMarkazChildMap = new HashMap<Long, List<ListOrderedMap<String, Object>>>();
			List<AccountingMarkazEntity> accountingMarkazList = accountingMarkazService
					.getActiveAccountingMarkaz(saalMaaliEntity, topOrganList);
			...
			organizationalAccountingMarkazChildMap.put(saalMaaliEntity.getId() + "_" + organId,
					accountingMarkazChildMap);
		}
	...
	public static List<ListOrderedMap<String, Object>> getRootHesabs(SaalMaaliEntity saalMaaliEntity, Long organId) {
		List<ListOrderedMap<String, Object>> rootHesabsList = organizationalRootHesabsMap
				.get(saalMaaliEntity.getId() + "_" + organId);

		if (rootHesabsList == null) {
			rootHesabsList = hesabKolService.getRootHesabs(saalMaaliEntity, organId);
