...
public class HesabTreeUtil {
	...
	public static HesabVO addHesabKolToHesabVOs(HesabKolEntity hesabKolEntity, List<HesabVO> hesabVOs) {
		for (HesabVO hesabVO : hesabVOs) {
			if (hesabVO.getHesabType().equals(HesabKolEntity.class.getSimpleName())
					&& Long.valueOf(hesabVO.getEntityId()).equals(hesabKolEntity.getId()))
				return hesabVO;
		}
	...
	public static void addHesabMoeensToHesabHierarchy(HesabVO hesabKolVO, List<HesabMoeenEntity> activeMoeens,
			List<HesabVO> hesabVOs, SaalMaaliEntity saalMaaliEntity, List<Long> topOrganList) {
	...
	private static HesabVO addHesabMoeenToHesabHierarchy(HesabVO hesabKolVO, HesabMoeenEntity hesabMoeenEntity,
			List<HesabVO> hesabVOs, SaalMaaliEntity saalMaaliEntity, List<Long> topOrganList) {
		...
		for (HesabVO hkVO : hesabVOs) {
			...
			for (HesabVO hesabMoeenVO : hesabMoeenVOs) {
				if (hesabMoeenVO.getHesabType().equals(HesabMoeenEntity.class.getSimpleName())
						&& Long.valueOf(hesabMoeenVO.getEntityId()).equals(hesabMoeenEntity.getId())) {
		...
		if (!found) {
			localHesabMoeenVO = new HesabVO(hesabMoeenEntity, HesabMoeenEntity.class.getSimpleName(),
					"folder-vectors-icon16.png");
		...
		for (HesabTafsiliEntity hesabTafsili : activeTafsilies) {
			HesabVO tafsiliVO = addHesabTafsiliToHesabMoeen(localHesabMoeenVO, hesabTafsili,
					"folder-documents-icon.png", hesabVOs);

			Set<HesabTafsiliEntity> tafsiliChilds = hesabTafsili.getChilds();
			for (HesabTafsiliEntity childTafsiliEntity : tafsiliChilds) {
				addHesabTafsilisShenavarToHesabHierarchy(tafsiliVO, childTafsiliEntity, "1365270554_stock_group.png",
						hesabVOs);
			}
	...
	public static HesabVO addHesabMoeenToHesabKol(HesabVO hesabKolVO, HesabMoeenEntity hesabMoeenEntity,
			List<HesabVO> hesabVOs) {
	...
	public static HesabVO addHesabTafsiliToHesabMoeen(HesabVO hesabMoeenVO, HesabTafsiliEntity hesabTafsili,
			String icon, List<HesabVO> hesabVOs) {
		...
		for (HesabVO hkVO : hesabVOs) {
			...
			for (HesabVO hmVO : hesabMoeenVOs) {
				...
				for (HesabVO htVO : htVOs) {
					if (htVO.getHesabType().equals(HesabTafsiliEntity.class.getSimpleName())
							&& Long.valueOf(htVO.getEntityId()).equals(hesabTafsili.getId())
							&& htVO.getParent().getHesabType().equals(HesabMoeenEntity.class.getSimpleName())
							&& htVO.getParent().getEntityId().equals(hesabMoeenVO.getEntityId())) {
						tafsiliVO = htVO;
						found = true;
						break;
					}
...
//	}
	...
	public static void addHesabTafsilisToHesabMoeen(List<HesabVO> hesabVOs, HesabTafsiliEntity hesabTafsiliEntity) {
		...
		for (MoeenTafsiliEntity moeenTafsiliEntity : moeenTafsili) {
			...
			HesabVO hesabTafsiliVO = addHesabTafsiliToHesabMoeen(hesabMoeenVO, hesabTafsiliEntity,
					"folder-documents-icon.png", hesabVOs);
		}
	}

	public static void addHesabShenavarsToHesabMoeen(List<HesabVO> hesabVOs,
			HesabTafsiliEntity parentHesabTafsiliEntity, HesabTafsiliEntity childHesabTafsili) {
		...
		for (MoeenTafsiliEntity moeenTafsiliEntity : moeenTafsili) {
			...
			HesabVO hesabTafsiliVO = addHesabTafsiliToHesabMoeen(hesabMoeenVO, parentHesabTafsiliEntity,
					"folder-documents-icon.png", hesabVOs);
			addHesabTafsilisShenavarToHesabHierarchy(hesabTafsiliVO, childHesabTafsili, "1365270554_stock_group.png",
					hesabVOs);
		}
	}
	...
	public static void addHesabTafsilisShenavarToHesabHierarchy(HesabVO HesabTafsiliVO,
			...
			for (HesabVO hmVO : hesabMoeenVOs) {
				...
				for (HesabVO htVO : htVOs) {
					...
					for (HesabVO hesabVO : htsVOs) {
						if (hesabVO.getHesabType().equals(HesabTafsiliEntity.class.getSimpleName() + "Shenavar")
								&& Long.valueOf(hesabVO.getEntityId()).equals(hesabTafsiliShenavar.getId())
								&& hesabVO.getParent().getHesabType().equals(HesabTafsiliEntity.class.getSimpleName())
								&& hesabVO.getParent().getEntityId().equals(HesabTafsiliVO.getEntityId())) {
		...
		if (!found) {
			tafsiliShenavarVO = new HesabVO(hesabTafsiliShenavar, HesabTafsiliEntity.class.getSimpleName() + "Shenavar",
					icon);
